...
from utils import IndexSelector   # 必须：IndexSelector 类定义在 utils.py 中

selector = joblib.load("models/feature_selector.pkl")   # utils.IndexSelector
model_b2 = joblib.load("models/final_model_B2.pkl")     # Pipeline(StandardScaler + SVR)
model_b1 = joblib.load("models/final_model_B1.pkl")     # Pipeline(StandardScaler + SVR)
...
> **注意：** `from utils import IndexSelector` 是必须的一行。`feature_selector.pkl` 内部存储了 `utils.IndexSelector` 的类路径，加载时 Python 需要能 import 到该类。预测脚本须与 `utils.py` 在同一目录，或将项目目录加入 `sys.path`。
